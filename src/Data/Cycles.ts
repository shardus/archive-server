...
function isSameCyceInfo(info1: Cycle, info2: Cycle): boolean {
...
export async function getNewestCycleFromConsensors(
  ...
  const newestCycle = await Utils.robustQuery(activeNodes, queryFn, isSameCyceInfo)
...
export async function getNewestCycleFromArchivers(activeArchivers: State.ArchiverNodeInfo[]): Promise<Cycle> {
  ...
  const cycleInfo = await Utils.robustQuery(activeArchivers, queryFn, isSameCyceInfo)
