...
function updateNodeList(cycle: Cycle) {
    ...
    const combinedArchivers = [...joinedArchivers, ...refreshedArchivers]

    for (let archiver of combinedArchivers) {
        let foundArchiver = State.activeArchivers.find(a => a.publicKey === archiver.publicKey)
        if (!foundArchiver) {
            State.activeArchivers.push(archiver)
